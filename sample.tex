...
\begin{definition}[Discrete Laplace mechanism for multinomial data: \texttt{DiscLapU}]\label{def:DiscLapU_formal}
	Given a pooled raw multinomial sample $\{X_i\}_{i \in [n]}$ with $\alphabetSize$ categories, fix the privacy level $\privacyParameter>0$. Each data owner adds noise to their data point, with the noise distribution parametrized by
	$$\discLapUParam := e^{
		-
